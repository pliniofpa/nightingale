...
function SidebarProvider({
  ...
  const wrapperStyle: SidebarWrapperStyle = {
...
const mobileVisibility = (open: boolean): string =>
  open ? 'pointer-events-auto visible' : 'pointer-events-none invisible';
const mobileOpacity = (open: boolean): string => (open ? 'opacity-100' : 'opacity-0');
const mobileTransform = (open: boolean): string => (open ? 'translate-x-0' : '-translate-x-full');
const desktopCollapsible = (state: string, collapsible: string): string =>
  state === 'collapsed' ? collapsible : '';
const desktopGapVariant = (variant: string): string =>
  variant === 'floating' || variant === 'inset'
    ? 'group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4)))]'
    : 'group-data-[collapsible=icon]:w-(--sidebar-width-icon)';
const desktopContainerVariant = (variant: string): string =>
  variant === 'floating' || variant === 'inset'
    ...
    <>
      ...
      {!isMobile && (
        <div
          ...
          data-state={state}
          data-collapsible={desktopCollapsible(state, collapsible)}
        ...
        >
          ...
          <div
            ...
            className={cn(
              ...
              desktopContainerVariant(variant),
              className,
            )}
            {...props}
          >
